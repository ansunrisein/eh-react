import React, {useCallback, useState} from 'react'
import {Meta, Story} from '@storybook/react'
import {UpdateProfileVariables} from '@eh/react/features/user/graphql/types/UpdateProfile'
import {Settings, SettingsProps} from './Settings'

export default {
  component: Settings,
  title: 'user/views/Settings',
  parameters: {layout: 'centered'},
  argTypes: {
    defaultProfileFormValues: {table: {disable: true}},
    onProfileFormSubmit: {table: {disable: true}},
    onAvatarSelected: {table: {disable: true}},
    onRemoveAvatar: {table: {disable: true}},
    avatar: {
      control: {type: 'text'},
      defaultValue:
        'https://avatars.githubusercontent.com/u/31159587?s=400&u=56eb4d76bbeae350f4d2699eca2ec364da11a2cb&v=4',
    },
  },
} as Meta

const upload = (file: File): Promise<string> =>
  new Promise(resolve => {
    const reader = new FileReader()
    reader.readAsDataURL(file)
    reader.addEventListener('load', () => resolve(String(reader.result)))
  })

const initialAvatar =
  'data:image/jpeg;base64,'

const initialValues = {nickname: 'Capitalize(@)ntonchik', name: 'AntonNina'}

export const Controlled: Story<SettingsProps> = () => {
  const [defaultValues, setDefaultValues] = useState<UpdateProfileVariables>(initialValues)
  const [defaultAvatar, setDefaultAvatar] = useState<string | null>(initialAvatar)
  const [avatar, setAvatar] = useState<string | null>(defaultAvatar)
  const [loading, setLoading] = useState(false)

  const uploadAvatar = useCallback((file: File) => {
    setLoading(true)
    upload(file)
      .then(setAvatar)
      .finally(() => setLoading(false))
  }, [])

  const removeAvatar = () => setAvatar(null)

  return (
    <Settings
      defaultProfileFormValues={defaultValues}
      onProfileFormSubmit={setDefaultValues}
      defaultAvatar={defaultAvatar}
      avatar={avatar}
      isAvatarUploading={loading}
      onAvatarSelected={uploadAvatar}
      onAvatarSubmit={setDefaultAvatar}
      onRemoveAvatar={removeAvatar}
    />
  )
}

Controlled.argTypes = {
  avatar: {table: {disable: true}},
  defaultAvatar: {table: {disable: true}},
  onAvatarSubmit: {table: {disable: true}},
  isAvatarUploading: {table: {disable: true}},
}
Controlled.parameters = {controls: {hideNoControlsWarning: true}}

export const Usual: Story<SettingsProps> = props => <Settings {...props} />
